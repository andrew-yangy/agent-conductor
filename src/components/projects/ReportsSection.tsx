  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 py-1.5 px-2">
        ...
        <span className="text-xs font-medium flex-1 truncate">{report.title}</span>
        {report.sourceDirective && (
          <span className="text-[9px] text-muted-foreground/60 bg-secondary px-1 py-0 rounded shrink-0">
            {report.sourceDirective}
          </span>
        )}
        <span className="text-[10px] text-muted-foreground/60 shrink-0">{report.createdAt?.split('T')[0] ?? ''}</span>
        <ChevronRight className={cn('h-3 w-3 text-muted-foreground/50 transition-transform', expanded && 'rotate-90')} />
      </div>
      {!expanded && report.contentSummary && (
...
export default function ReportsSection({ reports }: { reports: ArtifactRecord[] }) {

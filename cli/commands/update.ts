...
function backupPath(filePath: string, projectPath: string, backupDir: string): void {
...
function readExistingAgents(projectPath: string): Array<{ name: string; title: string; role: string }> {
  ...
  for (const registryPath of paths) {
    try {
      const data = JSON.parse(fs.readFileSync(registryPath, 'utf-8')) as {
        agents: Array<{ id: string; name: string; title: string; role: string }>;
      };
      return data.agents
        .filter(a => a.id !== 'ceo')
...
export async function runUpdate(flags: Record<string, string | boolean>): Promise<void> {
  if (flags['help']) {
    printUpdateHelp();
    process.exit(0);
  }
